                ...
                if (pBody)
                {
                    ...
                    if (SUCCEEDED(hr) && bTracked)
                    {
                        ...
                        if (SUCCEEDED(hr))
                        {
							...
							D2D1_POINT_2F leftHand_trackPos = BodyToScreen(joints[JointType_HandLeft].Position, device_width, device_height);
							D2D1_POINT_2F rightHand_trackPos = BodyToScreen(joints[JointType_HandRight].Position, device_width, device_height);
                            DrawHand(leftHandState, jointPoints[JointType_HandLeft], leftHand_trackPos, true);
                            DrawHand(rightHandState, jointPoints[JointType_HandRight], rightHand_trackPos, false);
                        }
                    }
                }
            }
...
void CBodyBasics::DrawBody(const Joint* pJoints, const D2D1_POINT_2F* pJointPoints)
{
	...
	DrawBone(pJoints, pJointPoints, JointType_Head, JointType_Neck);
		...
		m_pCoordinateMapper->MapCameraPointToColorSpace(pJoints[JointType_Head].Position, &colorPoint);
		int width = GetSystemMetrics(SM_CXSCREEN);
		int height = GetSystemMetrics(SM_CYSCREEN);
		//SetCursorPos(colorPoint.X / 1919 * width, colorPoint.Y / 1079 * height);
		m_mouseManager.set_pos_head(colorPoint.X / 1919 * width, colorPoint.Y / 1079 * height);
...
void CBodyBasics::DrawHand(HandState handState, const D2D1_POINT_2F& handPosition, const D2D1_POINT_2F& trackPos, bool isLeft)
{
	...
	if (!isLeft ) 
		m_mouseManager.set_pos_hand(trackPos.x, trackPos.y);
}
